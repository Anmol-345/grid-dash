  ...
  generate(
    ...
    while (targets.length < count && attempts < maxAttempts) {
      ...
      if (useDirectionalSpawn) {
        ...
        // Gridshot spawn bounds: player faces -Z (yaw=0)
        // X = [-5, 5] lateral, Y = [0.5, 3.5] height, Z = [-10, -2] in front
        x = roomX + (Math.random() * 10 - 5); // [-5, 5]
        y = 0.5 + Math.random() * 3.0;         // [0.5, 3.5]
        z = roomZ - (2 + Math.random() * 8);   // [-10, -2]
      }
