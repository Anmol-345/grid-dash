...
export default class Controls {
  ...
  public update(deltaTime: number) {
    ...
    // ── Aim-trainer mode: player always stands on the floor (Y = 0). ──
    // We skip the Rapier character controller entirely and apply horizontal
    // movement directly. This is reliable, jitter-free, and avoids all the
    // async-init timing issues that caused the player to fall through the floor.
    this.yawObject.position.addScaledVector(this.velocity, deltaTime);
    this.yawObject.position.y = 0;   // Clamp to floor — never leaves ground
    this.velocityY = 0;
    this.onGround = true;

    // Player is always grounded — no jump or gravity needed for an aim trainer
