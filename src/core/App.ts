...
import { exportSession } from "@/systems/SessionExporter";
...
function createEmptyStats(): StoredStats {
  ...
  private readonly statsStorageKey = "grindshotScenarioStats";
  ...
  constructor(ui?: UIController, options?: { disableServer?: boolean; editorMode?: boolean }) {
    ...
    const initPlayer = async () => {
      console.log("[App] ⚠️ initPlayer called! isEditorMode:", this.isEditorMode);
      ...
      if (!this.isEditorMode) {
        this.controls.initPlayerRoom(0, 0);
        this.scene.initPlayerRoom(0, 0);
        ...
        this.controls.teleportTo(0, 0, 0, 0);
  ...
  startTimer() {
    ...
    // gameStartTime stays null — the 60-second clock begins on first shot.
    // (see recordShotFired below)
  ...
  update(_deltaTime: number) {
    ...
    if (this.gameStartTime !== null) {
      ...
      this.timeRemaining = Math.max(0, GAME_DURATION_SECONDS - elapsed);
  ...
  private endSession() {
    ...
    // Tell UI we are done (you can add a specific event or use pause to show results)
  ...
  public checkCrosshairIntersections(): boolean {
    const objects = this.targets as unknown as import("three").Object3D[];
    ...
    if (!hitTarget.shootable || hitTarget.animating || !hitTarget.visible) return false;

    // Record hit in telemetry
    const now = performance.now();
    const timeSinceLastHit = this.lastHitTime ? now - this.lastHitTime : 0;
    this.lastHitTime = now;
    telemetry.recordHit(timeSinceLastHit);
    try {
    ...
    // Gridshot instantly spawns a replacement
    hitTarget.absorbAndDisappear(() => {
      // Spawn 1 replacement
      const newTargets = this.scene.targetManager.generateTargets(1, 0, 0, 0.4);
      if (newTargets.length > 0) {
        newTargets[0].makeShootable();
        this.targets.push(newTargets[0]);
      }
      
      // Clean up the destroyed target from our array
  ...
  private recordShotFired() {
    this.shotsFired += 1;
    // Start the 60-second countdown on the very first shot
    if (this.gameRunning && this.gameStartTime === null) {
      this.gameStartTime = performance.now();
      telemetry.reset(); // fresh telemetry from shot 1
    }
  }
  ...
  public startGame(scenarioId: string = "gridshot-3") {
    console.log("[App] Starting game core...");
    this.scene.initPlayerRoom(0, 0); // local player coordinates
  ...
  private async startScenarioById(scenarioId: string) {
    ...
    const spawnYaw = 0; // Always face same direction (0 degrees = -Z)
    ...
    if (!this.isEditorMode) {
      ...
      // Generate procedural targets using default Gridshot bounds
  ...
  public setPaused(paused: boolean) {
    ...
    if (paused) {
      this.collisionSystem.disablePhysics();
      // Reset gameStartTime to pause the timer (if implementing pause properly)
      // For now, gridshot trainers usually don't let you pause the timer cleanly or they reset.
      // We will just let the timer account for it by adjusting gameStartTime when unpausing.
    } else {
      this.collisionSystem.enablePhysics();
      if (this.gameRunning && this.gameStartTime === null) {
        // First unpause = start the round
        this.gameStartTime = performance.now();
        telemetry.reset();
      } else if (this.gameRunning && this.gameStartTime !== null) {
        // Resuming from pause: adjust gameStartTime so we don't lose time
        // Actually, for simplicity we can just track remaining time if we want,
        // but `timeRemaining` is already the source of truth if we update it.
        this.gameStartTime = performance.now() - (GAME_DURATION_SECONDS - this.timeRemaining) * 1000;
      }
    }
  }
}
